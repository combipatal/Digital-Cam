...
### 마. 배경 RAM 경고 제거
- ModelSim에서 `bg_buffer_ram2` 포트 A가 주소 범위를 벗어났다는 경고가 반복적으로 발생한 원인을 추적했습니다. Bank2를 사용하지 않는 동안에도 14비트 주소선에 원본 주소의 하위 비트를 그대로 전달해 깊이(11264)보다 큰 값이 입력된 것이 원인이었습니다.
- `digital_cam_top.v`와 `test_digital_cam_top.v`에서 Bank2를 접근할 때만 유효 주소를 제공하고, 그 외에는 `0`으로 고정하도록 수정하여 경고를 제거했습니다. 읽기 포트도 동일한 방식으로 마스킹해 시뮬레이터가 불필요한 경고를 출력하지 않도록 했습니다.

### 바. 캐니 필터 테스트 경로 노출
- 테스트 전용 상위 모듈에서 `canny_value`, `canny_ready` 신호를 외부 포트로 노출하여 ModelSim 테스트벤치가 Canny 결과를 직접 캡처할 수 있도록 했습니다.
- 테스트벤치는 IR 모드 값을 `CANNY`(3'd3)로 설정하고, 덤프 대상 파일을 `px_value_canny.hex`로 변경해 Canny 엣지 맵을 바로 추출합니다.

### 사. 가우시안 필터 출력 경로 추가 (2025-10-15)
- 테스트 상위 모듈(`test_digital_cam_top.v`)에 `gaussian_value`, `gaussian_ready`, `pixel_valid` 포트를 신설해 가우시안 블러 결과와 유효 영역 정보를 외부에서 직접 수집할 수 있게 했습니다.
- 가우시안 결과는 내부 파이프라인과 동일한 7클럭 지연에 맞춰 정렬되며, VGA 출력 모드 `MODE_GAUSS(3'd6)`를 추가해 하드웨어에서도 바로 확인 가능합니다.
- 테스트벤치(`tb_digital_cam.v`)는 새 포트를 사용해 가우시안 모드로 구동하고, 활성 픽셀(`pixel_valid`) 동안 `px_value_gaussian.hex`에 640x480 프레임 전체를 덤프하도록 갱신했습니다.

### 아. 색 추적 테스트 경로 확장 (2025-10-15)
- `test_digital_cam_top.v`에 `color_track_mask`, `color_track_ready` 포트를 추가했습니다. 색 추적 모듈의 출력은 RGB→HSV 변환(5클럭)과 트래커(2클럭) 지연 후 즉시 노출되며, 별도의 추가 지연 없이 VGA 파이프라인과 자연스럽게 정렬됩니다.
- `tb_digital_cam.v`는 `MODE_COLOR(3'd4)`로 구동하며, `pixel_valid`가 활성화된 모든 픽셀을 `px_mask_color.hex`에 기록합니다. 실제 추적이 유효(`color_track_ready=1`)한 픽셀은 RGB565 순수 빨강(0xF800)으로, 그 외 영역은 검정으로 저장해 후처리에서 쉽게 식별할 수 있습니다.

### 자. 배경제거 테스트 시나리오 강화 (2025-10-16)
- `test_digital_cam_top.v`에서 배경제거 전경 마스크(`adaptive_fg_mask`)와 수직 동기(`vsync`)를 외부 포트로 노출해 시뮬레이션 벤치가 프레임 경계와 전경 여부를 직접 확인할 수 있도록 했습니다.
- `tb_digital_cam.v`는 두 장의 320x240 프레임을 순차적으로 주입해 첫 번째 프레임은 자동 배경 캡처, 두 번째 프레임은 비교 대상으로 사용합니다. `vsync` 상승 에지를 이용해 두 번째 프레임부터 결과를 캡처하며, 활성 픽셀 동안 전경은 파이프라인 정렬된 `pixel_rgb565` 값(원본 RGB565), 배경은 0으로 기록하여 `px_mask_bgsub.hex`로 덤프합니다.
...
0. 지속적인 문서 업데이트 (Continuous Documentation Update):  codex는 대화 중 발생하는 프로젝트의 중요한 변경사항이나 모듈 이해에 필수적인 내용을 스스로 판단하여 이 `AGENTS.md` 파일을 지속적으로 업데이트합니다.
