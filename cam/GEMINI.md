...
- **세부 동작 3 (고도화)**: 배경의 밝기에 따라 감지 민감도를 자동으로 조절한다. (어두운 곳에서는 예민하게, 밝은 곳에서는 둔감하게)
...
- **배경 픽셀 (빠른 적응)**: 현재 픽셀이 배경으로 판단되면, 상대적으로 빠른 학습률 (`α = 1/16` 또는 `1/8`)을 적용하여 조명 변화 등 환경 변화에 빠르게 대응합니다.
- **전경 픽셀 (느린 흡수)**: 전경으로 판단된 픽셀(화면에 들어온 정지 객체)에 대해서는 매우 느린 학습률 (`α = 1/256` 또는 `1/128`)을 적용하여, 시간이 지남에 따라 점차 배경의 일부로 흡수되도록 만듭니다.

#### 나. 외부 임계값 기반 비교
- `threshold_in` 포트를 통해 상위 모듈(`digital_cam_top.v`)에서 실시간으로 계산된 임계값을 입력받습니다.
- 현재 픽셀과 배경 픽셀의 밝기 차이(`abs_gray_diff`)가 이 `threshold_in` 값보다 크면 전경(Foreground)으로 판단합니다.

### 2.2. `digital_cam_top.v`: 동적 임계값 계산 및 전체 제어
상위 모듈인 `digital_cam_top`이 동적 임계값 계산의 핵심 두뇌 역할을 수행합니다.

#### 가. 동적 임계값 (Dynamic Threshold) 계산
- **개념**: 고정된 임계값 대신, 배경 픽셀의 밝기에 따라 실시간으로 임계값을 조절하여 오탐지를 줄입니다.
- **로직**:
    1. 배경 메모리에서 현재 좌표의 배경 픽셀(`rddata_bg`)을 읽어옵니다.
    2. 사람의 시각 특성을 반영한 Luma 공식(`밝기 = R*0.299 + G*0.587 + B*0.114`)을 하드웨어 연산에 맞게 구현하여 배경 픽셀의 정확한 밝기(`bg_luma`)를 계산합니다.
    3. `새로운 THRESHOLD = (계산된 bg_luma / 2) + IR리모컨_기본_임계값` 수식을 사용하여 최종 임계값을 결정합니다.
    4. 이 최종 임계값을 `adaptive_background.v` 모듈의 `threshold_in` 포트로 전달합니다.
- **효과**:
    - **밝은 영역**: `bg_luma`가 높으므로 THRESHOLD가 자동으로 높아져, 사소한 그림자나 노이즈를 무시합니다.
    - **어두운 영역**: `bg_luma`가 낮으므로 THRESHOLD가 자동으로 낮아져, 작은 움직임도 민감하게 감지합니다.

#### 나. 파라미터 설정 및 제어
- **기본 임계값 제어**: IR 리모컨을 통해 `bg_sub_threshold_btn` 값을 조절하여, 동적 임계값의 최소/기본 레벨을 사용자가 튜닝할 수 있도록 합니다.
- **자동 배경 초기화**: 시스템 시작 및 리셋 시 첫 프레임을 자동으로 배경으로 지정합니다.
