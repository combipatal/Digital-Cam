...
---
# 동작 감지 기능 분석 및 최종 구현 (Motion Detection Feature Analysis & Final Implementation) - 2025년 10월 10일

## 1. 기능 목표 (Feature Goal)
- **궁극적 목표**: 완전 자동화된 움직임 감지(Motion Detection) 기능 구현.
...
위 목표를 달성하기 위해 `adaptive_background.v`와 `digital_cam_top.v` 모듈이 다음과 같이 구현되었습니다.

### 2.1. `adaptive_background.v`: 이중 학습률을 이용한 배경 모델링 (Dual-Rate Background Modeling)
- **핵심 아이디어**: '전경'과 '배경' 픽셀에 각기 다른 학습률(α)을 적용하여 배경 모델을 항상 업데이트합니다.
- **배경 픽셀 (빠른 적응)**: 조명 변화 등 환경 변화에 빠르게 대응하기 위해 상대적으로 빠른 학습률 (`α = 1/16`)을 적용합니다. (`B_new = (15/16)*B_old + (1/16)*I_curr`)
- **전경 픽셀 (느린 흡수)**: 화면에 들어온 정지 객체를 점진적으로 배경에 흡수시키기 위해 매우 느린 학습률 (`α = 1/256`)을 적용합니다. (`B_new = (255/256)*B_old + (1/256)*I_curr`). 이 덕분에 오랫동안 움직이지 않는 객체는 서서히 사라집니다.

### 2.2. `digital_cam_top.v`: 출력 로직 및 제어 단순화 (Output Logic & Control Simplification)
- **움직임 감지 모드 출력 (IR 버튼 4)**: `adaptive_background.v` 모듈의 `is_foreground` 출력에 따라 최종 화면을 구성합니다.
  - `is_foreground == 1` (움직임 감지): 해당 픽셀을 **원본 영상 색상**으로 출력합니다.
  - `is_foreground == 0` (배경 또는 정지): 해당 픽셀을 **검은색 (`16'h0000`)**으로 출력합니다.
- **자동 배경 초기화**: IR 리모컨 5번 버튼을 사용한 수동 배경 캡처 기능이 **제거되었습니다**. 대신, 시스템 시작 또는 리셋 직후의 **첫 번째 프레임**을 자동으로 초기 배경으로 설정합니다. 이후 모든 배경 업데이트는 이중 학습률 로직에 의해 자동으로 수행됩니다.
